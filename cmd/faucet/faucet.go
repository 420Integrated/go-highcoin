...
// faucet is an Highcoin faucet backed by a light client.
package main
...
var (
	...
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Highcoins to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Highcoin and the faucet")
...
var (
	highcoin = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Highcoins", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Highcoin address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Highcoin light client.
type faucet struct {
...
// close terminates the Highcoin connection and tears down the faucet.
...
// apiHandler handles requests for Highcoin grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(wsconn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, highcoin),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Highcoin address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/420integrated/go-highcoin/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), highcoin)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, highcoin)
			peers := f.stack.Server().PeerCount()
...
// the uniqueness identifier (user id/username), username, avatar URL and Highcoin address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Highcoin address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Highcoin address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Highcoin address to fund on
// success.
...
// returning the username, avatar URL and Highcoin address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Highcoin address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Highcoin address,
