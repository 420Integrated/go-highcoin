...
// returned from docker inspect, parsed into a form easily usable by pupphigh.
type containerInfos struct {
	running bool              // Flag if the container is running currently
...
func inspectContainer(client *sshClient, container string) (*containerInfos, error) {
	// Check if there's a container running for the service
