...
// nodeDockerfile is the Dockerfile required to run an Highcoin node.
var nodeDockerfile = `
FROM highcoin/client-go:latest
...
// an Highcoin node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Highcoin node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 highcoin --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
