...
FROM highcoin/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--highstats", "{{.Highstats}}", "--highport", "{{.HighPort}}",     \
...
services:
  faucet:
    ...
    ports:
      - "{{.HighPort}}:{{.HighPort}}"
      - "{{.HighPort}}:{{.HighPort}}/udp"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Highstats":      config.node.highstats,
		"HighPort":       config.node.port,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"HighPort":       config.node.port,
		"HighName":       config.node.highstats[:strings.Index(config.node.highstats, ":")],
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
		"FaucetAmount":  config.amount,
		"FaucetMinutes": config.minutes,
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Funding amount (base tier)":   fmt.Sprintf("%d Highcoins", info.amount),
		"Funding cooldown (base tier)": fmt.Sprintf("%d mins", info.minutes),
		"Funding tiers":                strconv.Itoa(info.tiers),
		"Captha protection":            fmt.Sprintf("%v", info.captchaToken != ""),
		"Using Twitter API":            fmt.Sprintf("%v", info.twitterToken != ""),
		"Highstats username":            info.node.highstats,
	}
	if info.noauth {
...
// checkFaucet does a health-check against a faucet server to verify if
