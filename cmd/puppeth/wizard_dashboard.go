...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"highstats", "explorer", "wallet", "faucet"} {
	...
	// If we have highstats running, ask if to make the secret public or not
	if w.conf.highstats != "" {
		fmt.Println()
		fmt.Println("Include highstats secret on dashboard (y/n)? (default = yes)")
