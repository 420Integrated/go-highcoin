...
func (s *Suite) dial66(t *utesting.T) *Conn {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "high", Version: 66})
	return conn
}
...
func (c *Conn) read66() (uint64, Message) {
	...
	case (GetBlockHeaders{}).Code():
		highMsg := new(high.GetBlockHeadersPacket66)
		if err := rlp.DecodeBytes(rawData, highMsg); err != nil {
			return 0, errorf("could not rlp decode message: %v", err)
		}
		return highMsg.RequestId, GetBlockHeaders(*highMsg.GetBlockHeadersPacket)
	case (BlockHeaders{}).Code():
		highMsg := new(high.BlockHeadersPacket66)
		if err := rlp.DecodeBytes(rawData, highMsg); err != nil {
			return 0, errorf("could not rlp decode message: %v", err)
		}
		return highMsg.RequestId, BlockHeaders(highMsg.BlockHeadersPacket)
	case (GetBlockBodies{}).Code():
		highMsg := new(high.GetBlockBodiesPacket66)
		if err := rlp.DecodeBytes(rawData, highMsg); err != nil {
			return 0, errorf("could not rlp decode message: %v", err)
		}
		return highMsg.RequestId, GetBlockBodies(highMsg.GetBlockBodiesPacket)
	case (BlockBodies{}).Code():
		highMsg := new(high.BlockBodiesPacket66)
		if err := rlp.DecodeBytes(rawData, highMsg); err != nil {
			return 0, errorf("could not rlp decode message: %v", err)
		}
		return highMsg.RequestId, BlockBodies(highMsg.BlockBodiesPacket)
	case (NewBlock{}).Code():
		msg = new(NewBlock)
