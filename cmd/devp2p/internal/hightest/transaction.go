...
func sendSuccessfulTxWithConn(t *utesting.T, s *Suite, tx *types.Transaction, sendConn *Conn) {
	t.Logf("sending tx: %v %v %v\n", tx.Hash().String(), tx.SmokePrice(), tx.Smoke())
...
func unknownTx(t *utesting.T, s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce()+1, to, tx.Value(), tx.Smoke(), tx.SmokePrice(), tx.Data())
...
func invalidNonceTx(t *utesting.T, s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce()-2, to, tx.Value(), tx.Smoke(), tx.SmokePrice(), tx.Data())
	return signWithFaucet(t, txNew)
}
...
func hugeAmount(t *utesting.T, s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce(), to, amount, tx.Smoke(), tx.SmokePrice(), tx.Data())
	return signWithFaucet(t, txNew)
}

func hugeSmokePrice(t *utesting.T, s *Suite) *types.Transaction {
	tx := getNextTxFromChain(t, s)
	smokePrice := largeNumber(2)
	var to common.Address
	if tx.To() != nil {
		to = *tx.To()
	}
	txNew := types.NewTransaction(tx.Nonce(), to, tx.Value(), tx.Smoke(), smokePrice, tx.Data())
	return signWithFaucet(t, txNew)
}
...
func hugeData(t *utesting.T, s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce(), to, tx.Value(), tx.Smoke(), tx.SmokePrice(), largeBuffer(2))
	return signWithFaucet(t, txNew)
}
