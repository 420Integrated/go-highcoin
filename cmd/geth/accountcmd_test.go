...
func TestAccountListEmpty(t *testing.T) {
	highcoin := runHighcoin(t, "account", "list")
	highcoin.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	highcoin := runHighcoin(t, "account", "list", "--datadir", datadir)
	defer highcoin.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	highcoin := runHighcoin(t, "account", "new", "--lightkdf")
	defer highcoin.ExpectExit()
	highcoin.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	highcoin := runHighcoin(t, "account", "import", keyfile, "-password", passwordFile)
	defer highcoin.ExpectExit()
	highcoin.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	highcoin := runHighcoin(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	highcoin := runHighcoin(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer highcoin.ExpectExit()
	highcoin.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(highcoin.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	highcoin := runMinimalHighcoin(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(highcoin.StderrText(), m) {
...
// https://github.com/420integrated/go-highcoin/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	highcoin.SetTemplateFunc("keypath", func(file string) string {
