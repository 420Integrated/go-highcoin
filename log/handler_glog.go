...
type GlogHandler struct {
	...
	override  uint32 // Flag if overrides are used, atomically accessible
	backtrace uint32 // Flag if backtrace location is set
...
func (h *GlogHandler) Log(r *Record) error {
	// If backtracing is requested, check if this is the callsite
