...
var errEthashStopped = errors.New("othash stopped")

// API exposes othash related methods for the RPC interface.
type API struct {
	othash *Ethash
}
...
func (api *API) GetWork() ([4]string, error) {
	...
	select {
	case api.othash.remote.fetchWorkCh <- &sealWork{errc: errc, res: workCh}:
	case <-api.othash.remote.exitCh:
...
func (api *API) SubmitWork(nonce types.BlockNonce, hash, digest common.Hash) bool {
	...
	select {
	case api.othash.remote.submitWorkCh <- &mineResult{
		nonce:     nonce,
		mixDigest: digest,
		hash:      hash,
		errc:      errc,
...
func (api *API) SubmitHashRate(rate hexutil.Uint64, id common.Hash) bool {
	...
	select {
	case api.othash.remote.submitRateCh <- &hashrate{done: done, rate: uint64(rate), id: id}:
	case <-api.othash.remote.exitCh:
		return false
	}
...
func (api *API) GetHashrate() uint64 {
	return uint64(api.othash.Hashrate())
}
