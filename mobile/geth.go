...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.HighcoinGenesis != "" {
		...
		if config.HighcoinGenesis == RinkebyGenesis() {
	...
	if config.HighcoinEnabled {
		...
		ethConf.NetworkId = uint64(config.HighcoinNetworkID)
		ethConf.DatabaseCache = config.HighcoinDatabaseCache
		...
		if config.HighcoinNetStats != "" {
			if err := ethstats.New(rawStack, lesBackend.ApiBackend, lesBackend.Engine(), config.HighcoinNetStats); err != nil {
...
func (n *Node) GetHighcoinClient() (client *HighcoinClient, _ error) {
	...
	return &HighcoinClient{ethclient.NewClient(rpc)}, nil
}
