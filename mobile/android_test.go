...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Highcoin.LightScryptN, Highcoin.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.highcoin", new NodeConfig());
			node.start();
			...
			HighcoinClient ec = node.getHighcoinClient();
	...
	// signatures too. Regression test for go-highcoin issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows collectively
	...
	workspace, err := ioutil.TempDir("", "highcoin-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/highcoin/highcointest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Highcoin and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.420integrated", "github.com/420integrated/go-highcoin/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "highcoin.aar"), "highcoin.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "highcoin", "highcointest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.420integrated.highcointest"
