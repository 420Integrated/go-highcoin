...
// Process processes the state changes according to the Highcoin rules by running
// the transaction messages using the statedb and applying any rewards to both
// the processor (coinbase) and any included uncles.
//
// Process returns the receipts and logs accumulated during the process and
// returns the amount of smoke that was used in the process. If any of the
// transactions failed to execute due to insufficient smoke it will return an error.
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		receipt, err := applyTransaction(msg, p.config, p.bc, nil, gp, statedb, header, tx, usedSmoke, vmenv)
		if err != nil {
...
func applyTransaction(msg types.Message, config *params.ChainConfig, bc ChainContext, author *common.Address, gp *SmokePool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedSmoke *uint64, evm *vm.EVM) (*types.Receipt, error) {
	...
	*usedSmoke += result.UsedSmoke
	...
	// Create a new receipt for the transaction, storing the intermediate root and smoke used
	// by the tx.
	receipt := &types.Receipt{Type: tx.Type(), PostState: root, CumulativeSmokeUsed: *usedSmoke}
	if result.Failed() {
...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *SmokePool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedSmoke *uint64, cfg vm.Config) (*types.Receipt, error) {
	...
	return applyTransaction(msg, config, bc, author, gp, statedb, header, tx, usedSmoke, vmenv)
}
