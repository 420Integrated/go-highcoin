...
// stateObject represents an Highcoin account which is being modified.
//
...
type stateObject struct {
	...
	addrHash common.Hash // hash of highcoin address of the account
	data     Account
	db       *StateDB
...
// empty returns if the account is considered empty.
func (s *stateObject) empty() bool {
	return s.data.Nonce == 0 && s.data.Balance.Sign() == 0 && bytes.Equal(s.data.CodeHash, emptyCodeHash)
}

// Account is the Highcoin consensus representation of accounts.
...
// Return the smoke back to the origin. Used by the Virtual machine or Closures
func (s *stateObject) ReturnSmoke(smoke *big.Int) {}
