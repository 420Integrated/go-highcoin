...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (highdb.Database, *BlockChain, error) {
	var (
...
func testBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		receipts, _, usedSmoke, err := blockchain.processor.Process(block, statedb, vm.Config{})
		if err != nil {
			blockchain.reportBlock(block, receipts, err)
			return err
		}
		err = blockchain.validator.ValidateState(block, statedb, receipts, usedSmoke)
		if err != nil {
			blockchain.reportBlock(block, receipts, err)
			return err
		}
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	if full {
		...
		if blocks[2].Header().SmokeLimit != ncm.SmokeLimit() {
			t.Errorf("last  block smokeLimit mismatch: have: %d, want %d", ncm.SmokeLimit(), blocks[2].Header().SmokeLimit)
		}
	} else {
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, ethash.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
		...
		if i%3 == 2 {
			for j := 0; j < i%4+1; j++ {
				tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxSmoke, nil, nil), signer, key)
				if err != nil {
					panic(err)
				}
				block.AddTx(tx)
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	makeDb := func() (highdb.Database, func()) {
...
func TestChainTxReorgs(t *testing.T) {
	...
	postponed, _ := types.SignTx(types.NewTransaction(0, addr1, big.NewInt(1000), params.TxSmoke, nil, nil), signer, key1)
	swapped, _ := types.SignTx(types.NewTransaction(1, addr1, big.NewInt(1000), params.TxSmoke, nil, nil), signer, key1)
	...
	chain, _ := GenerateChain(gspec.Config, genesis, ethash.NewFaker(), db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastDrop, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr2), addr2, big.NewInt(1000), params.TxSmoke, nil, nil), signer, key2)

			gen.AddTx(pastDrop)  // This transaction will be dropped in the fork from below the split point
			gen.AddTx(postponed) // This transaction will be postponed till block #3 in the fork

		case 2:
			freshDrop, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr2), addr2, big.NewInt(1000), params.TxSmoke, nil, nil), signer, key2)
	...
	chain, _ = GenerateChain(gspec.Config, genesis, ethash.NewFaker(), db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxSmoke, nil, nil), signer, key3)
		...
		case 2:
			...
			freshAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxSmoke, nil, nil), signer, key3)
			gen.AddTx(freshAdd) // This transaction will be added exactly at reorg time

		case 3:
			futureAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxSmoke, nil, nil), signer, key3)
...
func benchmarkLargeNumberOfValueToNonexisting(b *testing.B, numTxs, numBlocks int, recipientFn func(uint64) common.Address, dataFn func(uint64) []byte) {
	...
	blockGenerator := func(i int, block *BlockGen) {
		...
		for txi := 0; txi < numTxs; txi++ {
			...
			tx, err := types.SignTx(types.NewTransaction(uniq, recipient, big.NewInt(1), params.TxSmoke, big.NewInt(1), nil), signer, testBankKey)
			if err != nil {
				b.Error(err)
			}
			block.AddTx(tx)
...
// checking that the smoke usage of a hot SLOAD and a cold SLOAD are calculated
// correctly.
func TestEIP2718Transition(t *testing.T) {
	...
	// Expected smoke is intrinsic + 2 * pc + hot load + cold load, since only one load is in the access list
	expected := params.TxSmoke + params.TxAccessListAddressSmoke + params.TxAccessListStorageKeySmoke + vm.SmokeQuickStep*2 + vm.WarmStorageReadCostEIP2929 + vm.ColdSloadCostEIP2929
	if block.SmokeUsed() != expected {
		t.Fatalf("incorrect amount of smoke spent: expected %d, got %d", expected, block.SmokeUsed())
