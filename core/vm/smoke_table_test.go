...
func TestMemorySmokeCost(t *testing.T) {
	tests := []struct {
		size     uint64
		cost     uint64
		overflow bool
	...
	for i, tt := range tests {
		v, err := memorySmokeCost(&Memory{}, tt.size)
		if (err == ErrSmokeUintOverflow) != tt.overflow {
			t.Errorf("test %d: overflow mismatch: have %v, want %v", i, err == ErrSmokeUintOverflow, tt.overflow)
		}
		if v != tt.cost {
			t.Errorf("test %d: smoke cost mismatch: have %v, want %v", i, v, tt.cost)
		}
	}
}
...
var eip2200Tests = []struct {
	...
	{1, 2306, "0x6001600055", 2306, 0, ErrOutOfSmoke},                            // 1 -> 1 (2300 sentry + 2xPUSH)
...
func TestEIP2200(t *testing.T) {
	for i, tt := range eip2200Tests {
		...
		_, smoke, err := vmenv.Call(AccountRef(common.Address{}), address, nil, tt.smokepool, new(big.Int))
		if err != tt.failure {
			t.Errorf("test %d: failure mismatch: have %v, want %v", i, err, tt.failure)
		}
		if used := tt.smokepool - smoke; used != tt.used {
			t.Errorf("test %d: smoke used mismatch: have %v, want %v", i, used, tt.used)
		}
		if refund := vmenv.StateDB.GetRefund(); refund != tt.refund {
			t.Errorf("test %d: smoke refund mismatch: have %v, want %v", i, refund, tt.refund)
		}
	}
}
