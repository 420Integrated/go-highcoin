...
type Genesis struct {
	...
	SmokeLimit   uint64              `json:"smokeLimit"   gencodec:"required"`
...
func SetupGenesisBlock(db highdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	...
	// Check if the genesis block is already written.
	if genesis != nil {
		hash := genesis.ToBlock(nil).Hash()
		if hash != stored {
			return genesis.Config, hash, &GenesisMismatchError{stored, hash}
...
func (g *Genesis) ToBlock(db highdb.Database) *types.Block {
	if db == nil {
	...
	head := &types.Header{
		...
		Extra:      g.ExtraData,
		SmokeLimit:   g.SmokeLimit,
		SmokeUsed:    g.SmokeUsed,
	...
	if g.SmokeLimit == 0 {
		head.SmokeLimit = params.GenesisSmokeLimit
	}
	if g.Difficulty == nil {
...
func (g *Genesis) Commit(db highdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db highdb.Database) *types.Block {
...
// GenesisBlockForTesting creates and writes a block in which addr has the given marleys balance.
func GenesisBlockForTesting(db highdb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the Highcoin main net genesis block.
...
// DeveloperGenesisBlock returns the 'highcoin --dev' genesis block.
