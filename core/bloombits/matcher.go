...
type Matcher struct {
	...
	running uint32 // Atomic flag if a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togither.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togither
				var orVector []byte
...
// the requested retrieval queue to be serviced togither with other sessions.
//
