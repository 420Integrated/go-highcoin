...
func (st *insertStats) report(chain []*types.Block, index int, dirty common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		context := []interface{}{
			"blocks", st.processed, "txs", txs, "msmoke", float64(st.usedSmoke) / 1000000,
			"elapsed", common.PrettyDuration(elapsed), "msmokeps", float64(st.usedSmoke) * 1000 / float64(elapsed),
