...
// Verify that Client implements the highcoin interfaces.
var (
	_ = highcoin.ChainReader(&Client{})
	_ = highcoin.TransactionReader(&Client{})
	_ = highcoin.ChainStateReader(&Client{})
	_ = highcoin.ChainSyncReader(&Client{})
	_ = highcoin.ContractCaller(&Client{})
	_ = highcoin.SmokeEstimator(&Client{})
	_ = highcoin.SmokePricer(&Client{})
	_ = highcoin.LogFilterer(&Client{})
	_ = highcoin.PendingStateReader(&Client{})
	// _ = highcoin.PendingStateEventer(&Client{})
	_ = highcoin.PendingContractCaller(&Client{})
)
...
func TestToFilterArg(t *testing.T) {
	...
	for _, testCase := range []struct {
		name   string
		input  highcoin.FilterQuery
		output interface{}
		err    error
	}{
		{
			"without BlockHash",
			highcoin.FilterQuery{
				Addresses: addresses,
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Highcoin Service
	config := &highconfig.Config{Genesis: genesis}
	config.Highash.PowMode = ethash.ModeFake
	highservice, err := high.New(n, config)
	if err != nil {
		t.Fatalf("can't create new highcoin service: %v", err)
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
		t.Fatalf("can't start test node: %v", err)
	}
	if _, err := highservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func TestHighClient(t *testing.T) {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == highcoin.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != highcoin.NotFound {
		t.Fatal("error should be highcoin.NotFound")
	}
}
...
func testStatusFunctions(t *testing.T, client *rpc.Client) {
	...
	// SuggestSmokePrice (should suggest 1 Gmarleys)
	smokePrice, err := ec.SuggestSmokePrice(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if smokePrice.Cmp(big.NewInt(1000000000)) != 0 {
		t.Fatalf("unexpected smoke price: %v", smokePrice)
	}
}
...
func testCallContract(t *testing.T, client *rpc.Client) {
	...
	smoke, err := ec.EstimateSmoke(context.Background(), msg)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if smoke != 21000 {
		t.Fatalf("unexpected smoke price: %v", smoke)
	}
	// CallContract
