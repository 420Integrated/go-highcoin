...
import (
	...
	"github.com/420integrated/go-highcoin/signer/storage"
)
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*highapi.SignTransactionResult, error)
...
type UIClientAPI interface {
	...
	OnApprovedTx(tx highapi.SignTransactionResult)
...
func logDiff(original *SignTxRequest, new *SignTxResponse) bool {
	...
	if g0, g1 := original.Transaction.Smoke, new.Transaction.Smoke; g0 != g1 {
		modified = true
		log.Info("Smoke changed by UI", "was", g0, "is", g1)
	}
	if g0, g1 := big.Int(original.Transaction.SmokePrice), big.Int(new.Transaction.SmokePrice); g0.Cmp(&g1) != 0 {
		modified = true
		log.Info("SmokePrice changed by UI", "was", g0, "is", g1)
	}
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*highapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := highapi.SignTransactionResult{Raw: data, Tx: signedTx}
