...
type handlerConfig struct {
	Database   highdb.Database            // Database for direct sync insertions
...
// newHandler returns a handler for all Highcoin chain management protocol.
...
// runHighPeer registers an high peer into the joint high/snap peerset, adds it to
...
func (h *handler) runHighPeer(peer *high.Peer, handler high.Handler) error {
	...
	// Execute the Highcoin handshake
	var (
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Highcoin handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Highcoin peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Highcoin peer registration failed", "err", err)
		return err
	}
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in high syncer", "err", err)
		return err
	}
	if snap != nil {
...
// runSnapExtension registers a `snap` peer into the joint high/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `high`, all subsystem registrations and lifecycle management will be done by
// the main `high` handler to prevent strange races.
...
func (h *handler) removePeer(id string) {
	...
	if peer == nil {
		logger.Error("Highcoin peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `high` peer if it exists
	logger.Debug("Removing Highcoin peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Highcoin peer removal failed", "err", err)
	}
...
func (h *handler) Start(maxPeers int) {
	...
	h.wg.Add(2)
	go h.chainSync.loop()
	go h.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with high/64.
}
...
func (h *handler) Stop() {
	...
	log.Info("Highcoin protocol stopped")
}
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	var (
		...
		txset = make(map[*highPeer][]common.Hash) // Set peer->hash to transfer directly
		annos = make(map[*highPeer][]common.Hash) // Set peer->hash to announce
