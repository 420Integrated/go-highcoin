...
var (
	...
	// testAddr is the Highcoin address of the tester account.
	testAddr = crypto.PubkeyToAddress(testKey.PublicKey)
)

// testBackend is a mock implementation of the live Highcoin message handler. Its
// purpose is to allow testing the request/reply workflows and wire serialization
// in the `high` protocol without actually doing any data processing.
...
func testGetNodeData(t *testing.T, protocol uint) {
	...
	generator := func(i int, block *core.BlockGen) {
		...
		case 0:
			...
			tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testAddr), acc1Addr, big.NewInt(10000), params.TxSmoke, nil, nil), signer, testKey)
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more highcoin to account #1.
			// acc1Addr passes it on to account #2.
			tx1, _ := types.SignTx(types.NewTransaction(block.TxNonce(testAddr), acc1Addr, big.NewInt(1000), params.TxSmoke, nil, nil), signer, testKey)
			tx2, _ := types.SignTx(types.NewTransaction(block.TxNonce(acc1Addr), acc2Addr, big.NewInt(1000), params.TxSmoke, nil, nil), signer, acc1Key)
			block.AddTx(tx1)
			block.AddTx(tx2)
		case 2:
