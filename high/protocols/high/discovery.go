...
// enrEntry is the ENR entry which advertises `high` protocol on the discovery.
...
// StartENRUpdater starts the `high` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `high` ENR entry based on the current state of the chain.
