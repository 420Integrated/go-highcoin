...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `high` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `high`.
...
// NodeInfo represents a short summary of the `high` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Highcoin network ID (420=HighCoin Mainnet, 421=Ruderalis Testnet)
...
// nodeInfo retrieves some `high` protocol metadata about the running host node.
...
// Handle is invoked whenever an `high` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `high`", "err", err)
			return err
		}
	}
}
...
var high64 = map[uint64]msgHandler{
...
var high65 = map[uint64]msgHandler{
	// old 64 messages
	GetBlockHeadersMsg: handleGetBlockHeaders,
	BlockHeadersMsg:    handleBlockHeaders,
	GetBlockBodiesMsg:  handleGetBlockBodies,
...
var high66 = map[uint64]msgHandler{
	// high64 announcement messages (no id)
	NewBlockHashesMsg: handleNewBlockhashes,
	NewBlockMsg:       handleNewBlock,
	TransactionsMsg:   handleTransactions,
	// high65 announcement messages (no id)
	NewPooledTransactionHashesMsg: handleNewPooledTransactionHashes,
	// high66 messages with request-id
