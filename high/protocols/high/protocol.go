...
// ProtocolName is the official short name of the `high` protocol used during
// devp2p capability negotiation.
const ProtocolName = "high"

// ProtocolVersions are the supported versions of the `high` protocol (first
// is primary).
var ProtocolVersions = []uint{HIGH66, HIGH65, HIGH64}

// protocolLengths are the number of implemented message corresponding to
// different protocol versions.
var protocolLengths = map[uint]uint64{HIGH66: 17, HIGH65: 17, HIGH64: 17}

// maxMessageSize is the maximum cap on the size of a protocol message.
const maxMessageSize = 10 * 1024 * 1024

const (
	// Protocol messages in high/64
	StatusMsg          = 0x00
	...
	// Protocol messages overloaded in high/65
...
// Packet represents a p2p message in the `high` protocol.
type Packet interface {
	Name() string // Name returns a string corresponding to the message type.
	Kind() byte   // Kind returns the message type.
}

// StatusPacket is the network packet for the status message for high/64 and later.
...
// GetBlockHeadersPacket represents a block header query over high/66
...
// BlockHeadersPacket represents a block header response over high/66.
...
// GetBlockBodiesPacket represents a block body query over high/66.
...
// BlockBodiesPacket is the network packet for block content distribution over high/66.
...
// BlockBodiesRLPPacket66 is the BlockBodiesRLPPacket over high/66
...
// GetNodeDataPacket represents a trie node data query over high/66.
...
// NodeDataPacket is the network packet for trie node data distribution over high/66.
...
// GetReceiptsPacket represents a block receipts query over high/66.
...
// ReceiptsPacket is the network packet for block receipts distribution over high/66.
...
// ReceiptsPacket66 is the high-66 version of ReceiptsRLPPacket
...
// PooledTransactionsPacket is the network packet for transaction distribution over high/66.
...
// PooledTransactionsRLPPacket66 is the high/66 form of PooledTransactionsRLPPacket
