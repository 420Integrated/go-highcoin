...
type accountResponse struct {
	...
	nodes highdb.KeyValueStore // Database containing the reconstructed trie nodes
...
// Syncer is an Highcoin account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db    highdb.KeyValueStore // Database to store the trie nodes into (and dedup)
...
// NewSyncer creates a new snapshot syncer to download the Highcoin state over the
// snap protocol.
func NewSyncer(db highdb.KeyValueStore, bloom *trie.SyncBloom) *Syncer {
	return &Syncer{
		db:    db,
		bloom: bloom,
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]common.Hash, slots [][][]byte, proof [][]byte) error {
	...
	var (
		dbs    = make([]highdb.KeyValueStore, len(hashes))
