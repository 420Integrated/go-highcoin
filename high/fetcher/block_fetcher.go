...
var (
	blockAnnounceInMeter   = metrics.NewRegisteredMeter("high/fetcher/block/announces/in", nil)
	blockAnnounceOutTimer  = metrics.NewRegisteredTimer("high/fetcher/block/announces/out", nil)
	blockAnnounceDropMeter = metrics.NewRegisteredMeter("high/fetcher/block/announces/drop", nil)
	blockAnnounceDOSMeter  = metrics.NewRegisteredMeter("high/fetcher/block/announces/dos", nil)

	blockBroadcastInMeter   = metrics.NewRegisteredMeter("high/fetcher/block/broadcasts/in", nil)
	blockBroadcastOutTimer  = metrics.NewRegisteredTimer("high/fetcher/block/broadcasts/out", nil)
	blockBroadcastDropMeter = metrics.NewRegisteredMeter("high/fetcher/block/broadcasts/drop", nil)
	blockBroadcastDOSMeter  = metrics.NewRegisteredMeter("high/fetcher/block/broadcasts/dos", nil)

	headerFetchMeter = metrics.NewRegisteredMeter("high/fetcher/block/headers", nil)
	bodyFetchMeter   = metrics.NewRegisteredMeter("high/fetcher/block/bodies", nil)

	headerFilterInMeter  = metrics.NewRegisteredMeter("high/fetcher/block/filter/headers/in", nil)
	headerFilterOutMeter = metrics.NewRegisteredMeter("high/fetcher/block/filter/headers/out", nil)
	bodyFilterInMeter    = metrics.NewRegisteredMeter("high/fetcher/block/filter/bodies/in", nil)
	bodyFilterOutMeter   = metrics.NewRegisteredMeter("high/fetcher/block/filter/bodies/out", nil)
)
...
type BlockFetcher struct {
	...
	fetchingHook       func([]common.Hash)               // Method to call upon starting a block (high/61) or header (high/62) fetch
	completingHook     func([]common.Hash)               // Method to call upon starting a block body fetch (high/62)
	importedHook       func(*types.Header, *types.Block) // Method to call upon successful header or block import (both high/61 and high/62)
}
