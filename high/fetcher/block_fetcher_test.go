...
var (
	...
	unknownBlock = types.NewBlock(&types.Header{SmokeLimit: params.GenesisSmokeLimit}, nil, nil, nil, trie.NewStackTrie(nil))
)
...
func makeChain(n int, seed byte, parent *types.Block) ([]common.Hash, map[common.Hash]*types.Block) {
	blocks, _ := core.GenerateChain(params.TestChainConfig, parent, ethash.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
		...
		if parent == genesis && i%3 == 0 {
			...
			tx, err := types.SignTx(types.NewTransaction(block.TxNonce(testAddress), common.Address{seed}, big.NewInt(1000), params.TxSmoke, nil, nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
