...
func (b *testBackend) RPCSmokeCap() uint64 {
	return 25000000
}
...
func TestTraceCall(t *testing.T) {
	...
	api := NewAPI(newTestBackend(t, genBlocks, genesis, func(i int, b *core.BlockGen) {
		...
		//    value: 1000 marleys
		//    fee:   0 marleys
		tx, _ := types.SignTx(types.NewTransaction(uint64(i), accounts[1].addr, big.NewInt(1000), params.TxSmoke, big.NewInt(0), nil), signer, accounts[0].key)
		b.AddTx(tx)
	}))

	var testSuite = []struct {
		blockNumber rpc.BlockNumber
		call        highapi.CallArgs
		config      *TraceConfig
		expectErr   error
		expect      interface{}
	}{
		// Standard JSON trace upon the genesis, plain transfer.
		{
			...
			config:    nil,
			expectErr: nil,
			expect: &highapi.ExecutionResult{
				Smoke:         params.TxSmoke,
				Failed:      false,
				ReturnValue: "",
				StructLogs:  []highapi.StructLogRes{},
			},
		},
...
func TestTraceTransaction(t *testing.T) {
	...
	if !reflect.DeepEqual(result, &highapi.ExecutionResult{
		Smoke:         params.TxSmoke,
		Failed:      false,
		ReturnValue: "",
		StructLogs:  []highapi.StructLogRes{},
	...
	var testSuite = []struct {
		...
		// Trace head block
		{
			...
			expect: []*txTraceResult{
				{
					Result: &highapi.ExecutionResult{
						Smoke:         params.TxSmoke,
						Failed:      false,
						ReturnValue: "",
						StructLogs:  []highapi.StructLogRes{},
