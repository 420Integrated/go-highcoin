...
type Tracer struct {
	inited bool // Flag if the context was already inited from the EVM
	...
	smokeValue    *uint   // Swappable smoke value wrapped by a log accessor
...
func New(code string, txCtx vm.TxContext) (*Tracer, error) {
	...
	tracer.ctx["smokePrice"] = txCtx.SmokePrice
	...
	tracer.vm.PushGoFunction(func(ctx *duktape.Context) int { ctx.PushUint(*tracer.smokeValue); return 1 })
	tracer.vm.PutPropString(logObject, "getSmoke")
...
func (jst *Tracer) CaptureStart(from common.Address, to common.Address, create bool, input []byte, smoke uint64, value *big.Int) error {
	jst.ctx["type"] = "CALL"
	if create {
		jst.ctx["type"] = "CREATE"
	}
...
func (jst *Tracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, smoke, cost uint64, memory *vm.Memory, stack *vm.Stack, rStack *vm.ReturnStack, rdata []byte, contract *vm.Contract, depth int, err error) error {
	if jst.err == nil {
		// Initialize the context if it wasn't done yet
		if !jst.inited {
			jst.ctx["block"] = env.Context.BlockNumber.Uint64()
			// Compute intrinsic smoke
			...
			intrinsicSmoke, err := core.IntrinsicSmoke(input, nil, jst.ctx["type"] == "CREATE", isHomestead, isIstanbul)
			if err != nil {
				return err
			}
			jst.ctx["intrinsicSmoke"] = intrinsicSmoke
			jst.inited = true
		}
...
func (jst *Tracer) CaptureFault(env *vm.EVM, pc uint64, op vm.OpCode, smoke, cost uint64, memory *vm.Memory, stack *vm.Stack, rStack *vm.ReturnStack, contract *vm.Contract, depth int, err error) error {
	if jst.err == nil {
...
func (jst *Tracer) CaptureEnd(output []byte, smokeUsed uint64, t time.Duration, err error) error {
	jst.ctx["output"] = output
	jst.ctx["time"] = t.String()
	jst.ctx["smokeUsed"] = smokeUsed
