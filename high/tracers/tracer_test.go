...
func (account) ReturnSmoke(*big.Int)                                  {}
...
func testCtx() *vmContext {
	return &vmContext{blockCtx: vm.BlockContext{BlockNumber: big.NewInt(1)}, txCtx: vm.TxContext{SmokePrice: big.NewInt(100000)}}
}

func runTrace(tracer *Tracer, vmctx *vmContext) (json.RawMessage, error) {
	env := vm.NewEVM(vmctx.blockCtx, vmctx.txCtx, &dummyStatedb{}, params.TestChainConfig, vm.Config{Debug: true, Tracer: tracer})
	var (
		startSmoke uint64 = 10000
		value           = big.NewInt(0)
	)
	contract := vm.NewContract(account{}, account{}, value, startSmoke)
	contract.Code = []byte{byte(vm.PUSH1), 0x1, byte(vm.PUSH1), 0x1, 0x0}

	tracer.CaptureStart(contract.Caller(), contract.Address(), false, []byte{}, startSmoke, value)
	ret, err := env.Interpreter().Run(contract, []byte{}, false)
	tracer.CaptureEnd(ret, startSmoke-contract.Smoke, 1, err)
	if err != nil {
		return nil, err
	}
...
func TestTracer(t *testing.T) {
	execTracer := func(code string) []byte {
		t.Helper()
		ctx := &vmContext{blockCtx: vm.BlockContext{BlockNumber: big.NewInt(1)}, txCtx: vm.TxContext{SmokePrice: big.NewInt(100000)}}
	...
	for i, tt := range []struct {
		...
		}, { // tests intrinsic smoke
			code: "{depths: [], step: function() {}, fault: function() {}, result: function(ctx) { return ctx.smokePrice+'.'+ctx.smokeUsed+'.'+ctx.intrinsicSmoke; }}",
			want: `"100000.6.21000"`,
		},
	} {
