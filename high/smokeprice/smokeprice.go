...
// smokeprice for newly created transaction.
func NewOracle(backend OracleBackend, params Config) *Oracle {
	blocks := params.Blocks
	if blocks < 1 {
		blocks = 1
		log.Warn("Sanitizing invalid smokeprice oracle sample blocks", "provided", params.Blocks, "updated", blocks)
	}
	percent := params.Percentile
	if percent < 0 {
		percent = 0
		log.Warn("Sanitizing invalid smokeprice oracle sample percentile", "provided", params.Percentile, "updated", percent)
	}
	if percent > 100 {
		percent = 100
		log.Warn("Sanitizing invalid smokeprice oracle sample percentile", "provided", params.Percentile, "updated", percent)
...
func (gpo *Oracle) SuggestPrice(ctx context.Context) (*big.Int, error) {
	...
	// If the latest smokeprice is still available, return it.
	gpo.cacheLock.RLock()
...
type transactionsBySmokePrice []*types.Transaction

func (t transactionsBySmokePrice) Len() int           { return len(t) }
func (t transactionsBySmokePrice) Swap(i, j int)      { t[i], t[j] = t[j], t[i] }
func (t transactionsBySmokePrice) Less(i, j int) bool { return t[i].SmokePriceCmp(t[j]) < 0 }
...
func (gpo *Oracle) getBlockPrices(ctx context.Context, signer types.Signer, blockNum uint64, limit int, result chan getBlockPricesResult, quit chan struct{}) {
	...
	copy(txs, blockTxs)
	sort.Sort(transactionsBySmokePrice(txs))
