...
// highHandler implements the high.Backend interface to handle the various network
// packets that are sent as replies or broadcasts.
type highHandler handler

func (h *highHandler) Chain() *core.BlockChain     { return h.chain }
func (h *highHandler) StateBloom() *trie.SyncBloom { return h.stateBloom }
func (h *highHandler) TxPool() high.TxPool          { return h.txpool }

// RunPeer is invoked when a peer joins on the `high` protocol.
func (h *highHandler) RunPeer(peer *high.Peer, hand high.Handler) error {
	return (*handler)(h).runHighPeer(peer, hand)
}

// PeerInfo retrieves all known `high` information about a peer.
func (h *highHandler) PeerInfo(id enode.ID) interface{} {
...
func (h *highHandler) Handle(peer *high.Peer, packet high.Packet) error {
	...
	default:
		return fmt.Errorf("unexpected high packet type: %T", packet)
	}
}

// handleHeaders is invoked from a peer's message handler when it transmits a batch
// of headers for the local node to process.
func (h *highHandler) handleHeaders(peer *high.Peer, headers []*types.Header) error {
...
func (h *highHandler) handleBodies(peer *high.Peer, txs [][]*types.Transaction, uncles [][]*types.Header) error {
...
func (h *highHandler) handleBlockAnnounces(peer *high.Peer, hashes []common.Hash, numbers []uint64) error {
...
func (h *highHandler) handleBlockBroadcast(peer *high.Peer, block *types.Block, td *big.Int) error {
