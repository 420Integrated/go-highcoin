...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [high/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    highdb.Database  // Database to state sync into (and deduplicate via)
	...
	snapSync       bool         // If to run state sync over the snap protocol
...
func New(checkpoint uint64, stateDb highdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() highcoin.SyncProgress {
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	// sync mode. Long term we could drop fast sync or merge the two togither,
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "high", p.version, "head", hash, "td", td, "mode", mode)
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // If the next request is pivot verification
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put togither a new list of blocks to import, which is a superset
