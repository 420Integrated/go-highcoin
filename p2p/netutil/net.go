...
// Contains reports if the given IP is contained in the list.
...
// IsLAN reports if an IP is a local network address.
...
// IsSpecialNetwork reports if an IP is located in a special-use network range
...
// CheckRelayIP reports if an IP relayed from the given sender IP
...
// SameNet reports if two IP addresses have an equal prefix of the given bit length.
...
// Contains if the given IP is contained in the set.
