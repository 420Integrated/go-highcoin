...
// Package highstats implements the network stats reporting service.
package highstats
...
import (
	...
	highproto "github.com/420integrated/go-highcoin/high/protocols/highh"
...
// backend encompasses the bare-minimum functionality needed for highstats reporting
...
// Service implements an Highcoin netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(highstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["high"]; info != nil {
		network = fmt.Sprintf("%d", info.(*highproto.NodeInfo).Network)
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the highstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to highstats", "latency", latency)
...
type blockStats struct {
	...
	SmokeUsed    uint64         `json:"smokeUsed"`
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to highstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to highstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to highstats", "count", pending)
...
type nodeStats struct {
	...
	SmokePrice int  `json:"smokePrice"`
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	if ok {
		...
		smokeprice = int(price.Uint64())
	} else {
		sync := s.backend.Downloader().Progress()
		syncing = s.backend.CurrentHeader().Number.Uint64() >= sync.HighestBlock
	}
	// Assemble the node stats and send it to the server
	log.Trace("Sending node details to highstats")
