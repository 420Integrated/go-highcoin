...
// Package highapi implements the general Highcoin API functions.
package highapi
...
type Backend interface {
	// General Highcoin API
	Downloader() *downloader.Downloader
	SuggestPrice(ctx context.Context) (*big.Int, error)
	ChainDb() highdb.Database
	AccountManager() *accounts.Manager
	ExtRPCEnabled() bool
	RPCSmokeCap() uint64        // global smoke cap for high_call over rpc: DoS protection
	...
	return []rpc.API{
		{
			Namespace: "high",
			Version:   "1.0",
			Service:   NewPublicHighcoinAPI(apiBackend),
			Public:    true,
		}, {
			Namespace: "high",
			Version:   "1.0",
