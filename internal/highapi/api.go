...
import (
	...
	"github.com/420integrated/go-highcoin/accounts/abi"
	"github.com/420integrated/go-highcoin/accounts/keystore"
	"github.com/420integrated/go-highcoin/accounts/scwallet"
...
// PublicHighcoinAPI provides an API to access Highcoin related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicHighcoinAPI struct {
	b Backend
}

// NewPublicHighcoinAPI creates a new Highcoin protocol API.
func NewPublicHighcoinAPI(b Backend) *PublicHighcoinAPI {
	return &PublicHighcoinAPI{b}
}

// SmokePrice returns a suggestion for a smoke price.
func (s *PublicHighcoinAPI) SmokePrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicHighcoinAPI) Syncing() (interface{}, error) {
...
func (s *PublicTxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		if to := tx.To(); to != nil {
			return fmt.Sprintf("%s: %v marleys + %v smoke × %v marleys", tx.To().Hex(), tx.Value(), tx.Smoke(), tx.SmokePrice())
		}
		return fmt.Sprintf("contract creation: %v marleys + %v smoke × %v marleys", tx.Value(), tx.Smoke(), tx.SmokePrice())
	}
	// Flatten the pending transactions
	for account, txs := range pending {
...
func (s *PrivateAccountAPI) SignTransaction(ctx context.Context, args SendTxArgs, passwd string) (*SignTransactionResult, error) {
	...
	if args.SmokePrice == nil {
		return nil, fmt.Errorf("smokePrice not specified")
	}
	if args.Nonce == nil {
...
// Sign calculates an Highcoin ECDSA signature for:
// keccack256("\x19Highcoin Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/420integrated/go-highcoin/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with high_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Highcoin Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/420integrated/go-highcoin/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Highcoin signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and highcoin.
...
// PublicBlockChainAPI provides an API to access the Highcoin blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Highcoin blockchain API.
...
// GetBalance returns the amount of marleys for the given address in the state of the
...
func (args *CallArgs) ToMessage(globalSmokeCap uint64) types.Message {
	...
	// Set default smoke & smoke price if none were set
	smoke := globalSmokeCap
	if smoke == 0 {
		smoke = uint64(math.MaxUint64 / 2)
	}
	if args.Smoke != nil {
		smoke = uint64(*args.Smoke)
	}
	if globalSmokeCap != 0 && globalSmokeCap < smoke {
		log.Warn("Caller smoke above allowance, capping", "requested", smoke, "cap", globalSmokeCap)
		smoke = globalSmokeCap
	}
	smokePrice := new(big.Int)
	if args.SmokePrice != nil {
		smokePrice = args.SmokePrice.ToInt()
	}
	value := new(big.Int)
	if args.Value != nil {
	...
	msg := types.NewMessage(addr, args.To, 0, value, smoke, smokePrice, data, accessList, false)
	return msg
}
...
func DoCall(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides map[common.Address]account, vmCfg vm.Config, timeout time.Duration, globalSmokeCap uint64) (*core.ExecutionResult, error) {
	...
	// or, in case of unmetered smoke, setup a context with a timeout.
	...
	msg := args.ToMessage(globalSmokeCap)
	...
	// Execute the message.
	gp := new(core.SmokePool).AddSmoke(math.MaxUint64)
...
// See: https://github.com/420integrated/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
func DoEstimateSmoke(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, smokeCap uint64) (hexutil.Uint64, error) {
	...
	// Determine the highest smoke limit can be used during the estimation.
	if args.Smoke != nil && uint64(*args.Smoke) >= params.TxSmoke {
		hi = uint64(*args.Smoke)
	} else {
		// Retrieve the block to act as the smoke ceiling
	...
	// Recap the highest smoke limit with account's available balance.
	if args.SmokePrice != nil && args.SmokePrice.ToInt().BitLen() != 0 {
		...
		allowance := new(big.Int).Div(available, args.SmokePrice.ToInt())
		...
		if allowance.IsUint64() && hi > allowance.Uint64() {
			...
			log.Warn("Smoke estimation capped by limited funds", "original", hi, "balance", balance,
				"sent", transfer.ToInt(), "smokeprice", args.SmokePrice.ToInt(), "fundable", allowance)
			hi = allowance.Uint64()
		}
	}
	...
	executable := func(smoke uint64) (bool, *core.ExecutionResult, error) {
		args.Smoke = (*hexutil.Uint64)(&smoke)

		result, err := DoCall(ctx, b, args, blockNrOrHash, nil, vm.Config{}, 0, smokeCap)
		if err != nil {
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	if args.SmokePrice == nil {
		...
		args.SmokePrice = (*hexutil.Big)(price)
	}
	if args.Value == nil {
	...
	if args.Smoke == nil {
		...
		args.Smoke = &estimated
		log.Trace("Estimate smoke usage automatically", "smoke", args.Smoke)
	}
	if args.ChainID == nil {
...
func (args *SendTxArgs) toTransaction() *types.Transaction {
	...
	if args.AccessList == nil {
		data = &types.LegacyTx{
			...
			Smoke:      uint64(*args.Smoke),
			SmokePrice: (*big.Int)(args.SmokePrice),
...
// FillTransaction fills the defaults (nonce, smoke, smokePrice) on a given unsigned transaction,
...
//
// https://github.com/420integrated/wiki/wiki/JSON-RPC#high_sign
...
func (s *PublicTransactionPoolAPI) Resend(ctx context.Context, sendArgs SendTxArgs, smokePrice *hexutil.Big, smokeLimit *hexutil.Uint64) (common.Hash, error) {
	...
	var price = matchTx.SmokePrice()
	if smokePrice != nil {
		price = smokePrice.ToInt()
	}
	var smoke = matchTx.Smoke()
	if smokeLimit != nil {
	...
	for _, p := range pending {
		...
		if err == nil && pFrom == sendArgs.From && s.signer.Hash(p) == wantSigHash {
			...
			if smokePrice != nil && (*big.Int)(smokePrice).Sign() != 0 {
				sendArgs.SmokePrice = smokePrice
			}
			if smokeLimit != nil && *smokeLimit != 0 {
				sendArgs.Smoke = smokeLimit
			}
...
// PublicDebugAPI is the collection of Highcoin APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Highcoin APIs exposed over the private
// debugging endpoint.
...
// Version returns the current highcoin protocol version.
...
func checkTxFee(smokePrice *big.Int, smoke uint64, cap float64) error {
	...
	feeHigh := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(smokePrice, new(big.Int).SetUint64(smoke))), new(big.Float).SetInt(big.NewInt(params.Highcoin)))
	feeFloat, _ := feeHigh.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f highcoin) exceeds the configured cap (%.2f highcoin)", feeFloat, cap)
	}
	return nil
}
