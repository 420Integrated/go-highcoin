...
import (
	...
	"github.com/420integrated/go-highcoin/accounts/external"
	"github.com/420integrated/go-highcoin/accounts/keystore"
	"github.com/420integrated/go-highcoin/accounts/scwallet"
	"github.com/420integrated/go-highcoin/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of highcoin is "highcoin". If no
	...
	oldHighcoinResourceWarning bool
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Highcoin", keep that.
	if name == "highcoin" || name == "highcoin-testnet" {
		name = "Highcoin"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "highcoin" instances.
var isOldHighcoinResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by highcoin 1.4 are used if they exist.
	if warn, isOld := isOldHighcoinResource[path]; isOld {
		oldpath := ""
		if c.name() == "highcoin" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldHighcoinResourceWarning, "Using deprecated resource file %s, please move this file to the 'highcoin' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-highcoin-keystore")
		ephemeral = keydir
	}
