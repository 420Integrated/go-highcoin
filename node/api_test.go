...
// checking if the HTTP server is started correctly.
func TestStartRPC(t *testing.T) {
	type test struct {
		...
		wantReachable bool // if the HTTP server should be reachable at all
		wantHandlers  bool // if RegisterHandler handlers should be accessible
		wantRPC       bool // if JSON-RPC/HTTP should be accessible
		wantWS        bool // if JSON-RPC/WS should be accessible
	}
...
// checkBodyOK checks if the given HTTP URL responds with 200 OK and body "OK".
...
// checkRPC checks if JSON-RPC works against the given URL.
