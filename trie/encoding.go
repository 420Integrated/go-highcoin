...
// 'terminator' byte of value 0x10 which indicates if or not the node at the key
// contains a value. Hex key encoding is used for nodes loaded in memory because it's
// convenient to access.
//
// COMPACT encoding is defined by the Highereum Yellow Paper (it's called "hex prefix
// encoding" there) and contains the bytes of the key and a flag. The high nibble of the
// first byte contains the flag; the lowest bit encoding the oddness of the length and
// the second-lowest encoding if the node at the key is a value node. The low nibble
...
// hasTerm returns if a hex key has the terminator flag.
