...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Highcoin
...
type wallet struct {
	...
	deriveChain     highcoin.ChainStateReader // Blockchain state reader to discover used account with
...
// if they are still present or if they malfunctioned.
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Highcoin account
...
// Contains implements accounts.Wallet, returning if a particular account is
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain highcoin.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
