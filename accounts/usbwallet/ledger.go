...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-high/master/doc/ethapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Highcoin address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Highcoin transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Highcoin app in browser mode", w.failure
	}
	if w.offline() {
		return "Highcoin app offline", w.failure
	}
	return fmt.Sprintf("Highcoin app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns if the wallet and the Highcoin app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Highcoin app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Highcoin app's version, will fail prior to v1.0.2
...
// and returning the Highcoin address located on that derivation path.
...
//
// Note, if the version of the Highcoin application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Highcoin app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Highcoin wallet app running
...
// ledgerDerive retrieves the currently active Highcoin address from a Ledger
...
//   Highcoin address length | 1 byte
//   Highcoin address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Highcoin hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Highcoin address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on if legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
	if chainID == nil {
		if txrlp, err = rlp.EncodeToBytes([]interface{}{tx.Nonce(), tx.SmokePrice(), tx.Smoke(), tx.To(), tx.Value(), tx.Data()}); err != nil {
			return common.Address{}, nil, err
		}
	} else {
		if txrlp, err = rlp.EncodeToBytes([]interface{}{tx.Nonce(), tx.SmokePrice(), tx.Smoke(), tx.To(), tx.Value(), tx.Data(), chainID, big.NewInt(0), big.NewInt(0)}); err != nil {
			return common.Address{}, nil, err
		}
	}
	...
	// Extract the Highcoin signature and do a sanity validation
