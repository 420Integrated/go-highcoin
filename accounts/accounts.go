...
// Package accounts implements high level Highcoin account management.
package accounts
...
// Account represents an Highcoin account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Highcoin account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain highcoin.ChainStateReader)
...
//   keccak256("\x19Highcoin Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Highcoin Signed Message:\n%d%s", len(data), string(data))
