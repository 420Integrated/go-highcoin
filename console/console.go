...
import (
	...
	"github.com/420integrated/go-highcoin/internal/jsre/deps"
	"github.com/420integrated/go-highcoin/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Highcoin requests through
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jhigh.Set("sign", personal.Get("sign"))
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines high.getBalance(high.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Highcoin instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Highcoin JavaScript console!\n\n"

	// Print some generic Highcoin metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + high.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + high.blockNumber + " (" + new Date(1000 * high.getBlock(high.blockNumber).timestamp) + ")\n";
		try {
